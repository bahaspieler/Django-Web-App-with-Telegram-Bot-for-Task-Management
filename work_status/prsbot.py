...
# fs ='/media/PRS'
# pre_data_p= os.path.join(fs,'3g_pre.csv')
# post_data_p= os.path.join(fs,'3g_post.csv')
# site_list_p = os.path.join(fs, '3g_site_list.csv')
# th_p = os.path.join(fs,'3g_threshold.csv')
# ultimate_p= os.path.join(fs, '3G Ultimate.xlsx' )

datas= prs.objects.get(pk=1)
pre_data_p = datas.pre_data.path

class prsdata():
    def three(prep,postp,thp, ultp, sitep):
        print("Reading Excel files....")
        df1 = pd.read_csv(prep, index_col=2, skiprows=6)
        df2 = pd.read_csv(postp, index_col=2, skiprows=6)
        print("Excel files has been read..")
        threshold_path = pd.read_csv(thp, index_col=False, header=None, skiprows=1)

        ultimate = pd.ExcelWriter(ultp, engine='openpyxl')
        ...
        site_path = pd.read_csv(sitep, index_col=False, header=None, skiprows=1)
        ...
        list3 = ['Time', 'RNC', 'Cell Name', 'NodeB Name', 'Integrity', '3G Cell Availability']

        list4 = ['Time', 'RNC', 'Cell Name']
        ...
        x = df_half.astype(str) + " <" + rounded_res.astype(str) + ">"
    ...
    def two():
        ...
        ultimate = pd.ExcelWriter("2g\\2G Ultimate_2.xlsx", engine='openpyxl')
        ...
        list3 = ['Time', 'GBSC', 'Cell Name', 'CellIndex', 'Site Name', 'Integrity', 'Cell availability']

        list4 = ['Time', 'GBSC', 'Cell Name']
        ...
        df_f['S3655:Number of configured TRXs in a cell'] = np.where(
            (df_f['S3655:Number of configured TRXs in a cell'] < trx_num), 'TRX Deleted',
            np.where((df_f['S3655:Number of configured TRXs in a cell'] == 8888), 'zero(no change)',
                     np.where((df_f['S3655:Number of configured TRXs in a cell'] > trx_num), 'TRX Added',
                              df_f['S3655:Number of configured TRXs in a cell'])))

        df_f['S3656:Number of available TRXs in a cell'] = np.where(
            (df_f['S3656:Number of available TRXs in a cell'] < trx_avail), 'TRX Down',
            np.where((df_f['S3656:Number of available TRXs in a cell'] == 8888), 'zero(no change)',
                     np.where((df_f['S3656:Number of available TRXs in a cell'] > trx_avail), 'Extra TRX',
                              df_f['S3656:Number of available TRXs in a cell'])))
    ...
    def lte():
        ...
        list3 = ['Time', 'Cell Name', 'LocalCell Id', 'Integrity', 'Cell Availablility (%)']

        list4 = ['Time', 'LocalCell Id', 'Cell Name']
